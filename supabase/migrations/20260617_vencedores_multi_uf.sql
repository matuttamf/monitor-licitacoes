-- Adiciona p_ufs TEXT[] para suportar filtro por macro-região (múltiplas UFs).
-- p_uf continua funcionando para compatibilidade; p_ufs tem precedência quando informado.
...
BEGIN
  ...
  WHERE r.nome_vencedor IS NOT NULL
    ...
    AND (
      -- p_ufs (macro-região) tem precedência sobre p_uf
      CASE
        WHEN p_ufs IS NOT NULL THEN r.estado = ANY(p_ufs)
        WHEN p_uf  IS NOT NULL THEN r.estado = p_uf
        ELSE TRUE
      END
    )
    AND (
