-- Corrige GROUP BY incompatível com SELECT: coalesce(cnpj,'') != coalesce(cnpj,nome)
-- PostgreSQL rejeita SELECT com expressão diferente do GROUP BY — causava erro 500 silencioso.
...
BEGIN
  ...
  RETURN QUERY
  SELECT
    -- usa MAX() para satisfazer o GROUP BY; cnpj é único por grupo
