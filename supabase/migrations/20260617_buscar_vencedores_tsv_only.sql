-- Corrige timeout: remove similarity() do WHERE (varredura completa sem índice).
-- Usa apenas tsv @@ v_query que tem índice GIN e é O(log n).
-- Para buscas sem tsquery válida, busca por ilike na descricao (limitado a 5k rows).
...
BEGIN
  ...
  WHERE r.nome_vencedor IS NOT NULL
    ...
    AND (
      ...
      -- Com tsquery válida: usa índice GIN (rápido)
      OR (v_query IS NOT NULL AND r.tsv @@ v_query)
      -- Fallback ilike apenas quando tsquery não gerou resultado utilizável
