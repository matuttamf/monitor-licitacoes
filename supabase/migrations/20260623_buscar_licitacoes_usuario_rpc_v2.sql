-- v2: corrige bug "SELECT DISTINCT com ORDER BY CASE WHEN"
-- PostgreSQL exige que expressões do ORDER BY estejam no SELECT quando há DISTINCT.
-- Solução: separar DISTINCT (CTE base) do ORDER BY (CTE ordered) usando CTEs.
...
BEGIN
  ...
  -- Página: DISTINCT primeiro (CTE base), ORDER BY depois (CTE ordered)
  ...
  FROM (
    ...
    FROM (
      -- Passo 2: ordenar + paginar o conjunto deduplicado
      SELECT * FROM (
        -- Passo 1: deduplicar licitações (DISTINCT separado do ORDER BY)
